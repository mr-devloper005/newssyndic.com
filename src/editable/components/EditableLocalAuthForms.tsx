...
const inputClass = 'h-[3.25rem] rounded-[1.25rem] border border-[color:rgba(84,107,65,0.18)] bg-[var(--slot4-warm)] px-4 text-base font-bold text-black outline-none transition placeholder:text-black/35 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-full bg-[var(--slot4-dark-bg)] px-6 text-xs font-black uppercase tracking-[0.2em] text-[var(--slot4-dark-text)] transition hover:bg-[var(--slot4-accent)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.25rem] border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
